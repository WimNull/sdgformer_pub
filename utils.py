...
class MetricLogger:
    ...
    def __str__(self):
        ...
        for name, meter in self.meters.items():
            meter_str.append(f"{name}: {str(meter)}")
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        if torch.cuda.is_available():
            log_msg = self.delimiter.join(
                [header, "[{0" + space_fmt + "}/{1}]", "eta: {eta}", "{meters}", "max mem: {memory:.0f}",]
            )
        else:
            log_msg = self.delimiter.join(
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0:
                ...
                if torch.cuda.is_available():
                    ...
                    print(log_msg.format(i, len(iterable), eta=eta_string, meters=str(self), memory=mem,)+self.otstr)
                else:
                    print(log_msg.format(i, len(iterable), eta=eta_string, meters=str(self), time=str(iter_time))+self.otstr)
            i += 1
            end = time.time()
...
def get_logger(name=None, filename=None, level=logging.DEBUG, format='%(asctime)s %(message)s', datefmt='%m-%d %H:%M', mode='w+', stdout=False):
